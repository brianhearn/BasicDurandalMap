...
var activeMupp; // current map units per pixel
var activeLayerId; // current map layer id being displayed
var activeSymbolRadiusInMapUnits; // the radius of the active layer's symbol in map units
var nudgeInProgress = false;
// nudge kernel
var nudgeKernel = [
    [0.7, 1],
    [0.5, 0.66],
...
nudgeKernel.forEach(function (r) { neighborSearchMultipleX += (r[0] * 3); });
var neighborSearchMultipleY = 0; // multiple of label size from anchor to search for neighbors
nudgeKernel.forEach(function (r) { neighborSearchMultipleY += (r[1] * 3); });
...
var moveLabel = function (dl) {
    ...
    var msg = { type: 'moveLabel', data: moveMsg };
    _this.postMessage(msg);
};
// send a message to the client to remove a label
var removeLabel = function (hash) {
    var dl = labelDictionary[hash];
    var remMsg = {
        hash: hash
    };
    var msg = { type: 'removeLabel', data: remMsg };
    _this.postMessage(msg);
...
var updateNeighbors = function (dl) {
    var dlCompare;
    var hash = dl.hash;
    var center = dl.mapAnchorLocation;
...
var makeNudgeReady = function (dl) {
    var p = dl.mapAnchorLocation;
...
var getMapBoundsOfLabel = function (dl, ignoreCache) {
    var b = boundsCache[dl.hash];
    if (!b || ignoreCache) {
        var p = dl.mapTextLocation;
        var x = p._x;
        var y = p._y;
...
var getMapBoundsOfLabelAnchor = function (dl) {
...
var createLabelText = function (activeColumnNames, valObj) {
    var txt = '';
...
rim.point.createFromObject = function (obj) { return new rim.point(obj.x, obj.y); };
rim.point.distance = function (x1, y1, x2, y2) {
    var dx = x1 - x2;
    var dy = y1 - y2;
...
rim.point.midpoint = function (x1, y1, x2, y2) { return new rim.point(x1 + ((x2 - x1) / 2), y1 + ((y2 - y1) / 2)); };
...
rim.envelope.createFromObject = function (obj) { return new rim.envelope(obj.minX, obj.minY, obj.maxX, obj.maxY); };
rim.envelope.createFromPoints = function (pt1, pt2) {
    var x1 = pt1._x;
    var y1 = pt1._y;
    var x2 = pt2._x;
    var y2 = pt2._y;
...
rim.envelope.createFromCenterAndMargins = function (centerPtX, centerPtY, marginX, marginY) { return new rim.envelope(centerPtX - marginX, centerPtY - marginY, centerPtX + marginX, centerPtY + marginY); };
rim.envelope.intersects = function (env1, env2) { return !(env1._minY > env2._maxY || env1._minX > env2._maxX || env1._maxY < env2._minY || env1._maxX < env2._minX); };
rim.envelope.intersection = function (env1, env2) {
...
rim.envelope.union = function (env1, env2) { return new rim.envelope(Math.min(env1._minX, env2._minX), Math.min(env1._minY, env2._minY), Math.max(env1._maxX, env2._maxX), Math.max(env1._maxY, env2._maxY)); };
//# sourceMappingURL=dynamicLabelsWorker.js.map